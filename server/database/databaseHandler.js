        ...
        pool.query("SELECT id FROM playlist where title=$1",[request.body.genre[0]], function (error, results, fields) {//fetches id from playlist table of specific song
            ...
            if(typeof results.rows[0] !== "undefined")
            {
                ...
                let alreadyExistCount=0;
                    ...
                    {
                        pool.query("INSERT INTO track (playlist_id, title, uri, master_id, image, country)VALUES ($1, $2, $3, $4, $5, $6)",[playlist_id, request.body.title, request.body.uri, request.body.master_id, request.body.thumb, request.body.country], function (error, results, fields) {
                            if (error) throw error;
                        });
                    }
                });
